...
Error_info F_P()
{
    if(cur.token_id == -1||cur.token_id == 10086) return Error_info();
    else return F_C();
}
Error_info F_C()
{
...
Error_info F_C_tmp()
{
    if(cur.token_id != -1&&cur.token_id != 10086) return F_C();
    return Error_info();
}
Error_info F_IN()
{
    if( cur.token_id == 15)
    {
        ...
        if(cur.info == "Int" || cur.info == "String" || cur.info == "Bool")
            ...
            //cout<<cur.info<<endl;
        //cout<<cur_type_name<<" "<<cur.info<<endl;
...
Error_info F_F_tmp()
{
    ...
    if(cur.token_id == 65)
    {
        ...
        _GET_NEXT_DATA;
        tmp_Error = F_E();
        if(tmp!="SELF_TYPE"&& !check_fa_type(cur_id_type, tmp)) return Error_info( cur.line_num, "Wrong return type;");
        if(tmp == "SELF_TYPE" && !check_fa_type(cur_id_type, cur_type_name))return Error_info( cur.line_num, "Wrong return type;");
        check_error;
        if( cur.token_id != 68 ) return default_synatax_error;
        _GET_NEXT_DATA;
        if( cur.token_id != 52 ) return default_synatax_error;
...
Error_info F_M_tmp()
{
   // string tmp_type_name= cur_type_name;
    ...
    if( cur.token_id == 54)
    {
        ...
        int no = ty->Find_method( cur.info);
        //cout<<ty->Tname<<endl;
        tm = method_pool +no;
            ...
            else cur_id_type = cur_type_name;
        tmp_Error = F_M_tmp(); check_error;
    }
    return Error_info();
}
